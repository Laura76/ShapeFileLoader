...
public class SHP_Header {
  ...
  private File file;
  private final static int SHP_MAGIC   = 9994;//文件代码
  private final static int SHP_VERSION = 1000;//版本
  
  private int SHP_file_length, SHP_shape_type;//文件长度-文件类型
  ...
  public void read(ByteBuffer bb) throws Exception{
    // MAIN FILE HEADER
    bb.order(ByteOrder.BIG_ENDIAN);//大字节
    // magic number
    int SHP_MAGIC_read   = bb.getInt( 0);//文件头的魔数即文件代码9994
    if( SHP_MAGIC_read != SHP_MAGIC ){
      throw new Exception("(ShapeFile) error: SHP_MAGIC = "+SHP_MAGIC+", File: "+file );
    }
    // file length
    SHP_file_length  = bb.getInt(24);//文件长度
    bb.order(ByteOrder.LITTLE_ENDIAN);//小字节
    int SHP_version_read      = bb.getInt(28);//文件版本1000
    if( SHP_version_read != SHP_VERSION ){
      throw new Exception("(ShapeFile) error: SHP_VERSION = "+SHP_VERSION+", File: "+file );
    }
    
    SHP_shape_type = bb.getInt(32);//shape类型
   
    try {
      shape_type = ShpShape.Type.byID(SHP_shape_type);//根据ID获取shapetype
    } catch (Exception e) {
      e.printStackTrace();
    }
