...
public class DBF_Field {
  ...
  public static final int SIZE_BYTES = 32;//每个字段占32个字节
  //@SuppressWarnings("unused") 表示该属性在方法或类中没有使用。添加此注解可以去除属性上的黄色警告！！！
  @SuppressWarnings("unused")
  private DBF_File parent_dbasefile;
  private int index = 0;
  private String DBF_field_name = "";//字段名称
  private char   DBF_field_type        ;//字段类型
  @SuppressWarnings("unused")
  private int    DBF_field_displacement;
  private int    DBF_field_length      ;//字段长度
  @SuppressWarnings("unused")
  ...
  public DBF_Field(DBF_File parent_dbasefile, ByteBuffer bb, int index){
    ...
    this.index = index;//字段的索引--即第几个字段
    
    // read data
    byte[] string_tmp = new byte[11];  //0-11-字段名称
    bb.get(string_tmp);
    try {
    ...
    DBF_field_type         = (char)bb.get   ();//字段类型
    DBF_field_displacement = bb.getInt();//系统保留
    DBF_field_length       = bb.get   () & 0xFF; // so we get values from 0-255.-字段长度
    DBF_field_flag         = bb.get   ();//小数位数
