...
public class DBF_File extends ShapeFileReader{
  ...
  //HEADER-文件头信息区
  private byte DBF_file_type;//文件类型，1字节
  private int  DBF_date_yy;
  private int  DBF_date_mm;
  private int  DBF_date_dd;//文件最后修改日期
  
  private int  DBF_number_of_records; // number of records.-记录总数
  private int  DBF_size_header_bytes; // size of header (bytes).-文件头长度
  private int  DBF_size_record_bytes; // size of record (bytes).-记录长度
  
  // FIELDS
  private DBF_Field[] DBF_fields;//字段数组（类似数据库中的表的字段）
  
  // RECORD TABLE
  private String[][] DBF_records;//记录集合
  ...
  public void read() throws Exception{
    //--------------------------------------------------------------------------
    // READ HEADER
    bb.order(ByteOrder.LITTLE_ENDIAN);//小字节
    DBF_file_type         = bb.get     ( 0);
    DBF_date_yy           = bb.get     ( 1)+1900;
    DBF_date_mm           = bb.get     ( 2);
    DBF_date_dd           = bb.get     ( 3);
    DBF_number_of_records = bb.getInt  ( 4);
    DBF_size_header_bytes = bb.getShort( 8);//文件头长度（文件头信息区和字段描述符）
    DBF_size_record_bytes = bb.getShort(10);//一条记录的字节长度
    ...
    int num_fields = (DBF_size_header_bytes-POS-1 )/DBF_Field.SIZE_BYTES; // cant this be easier???字段的个数
...
//      DBF_fields[i].print();
    ...
    byte DBF_header_terminator = bb.get(); // = 13 or 0x0D--字段描述区的结束符
